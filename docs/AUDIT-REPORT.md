...
| 5 | **Tabla `public.ingredientes` en producción (110 filas) confirmada huérfana** — ningún código del frontend la consulta; el motor real usa un archivo JS estático (`data/keto-inspector-ingredientes.js`). RLS original `USING (true)` (lectura pública sin restricción) | Tabla en producción, ver `docs/archive/sql-legacy/README.md` | Media — dato inerte expuesto sin restricción; **borrar la tabla es irreversible, pendiente de decisión explícita del usuario** |
